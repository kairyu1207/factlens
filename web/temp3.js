            ...
            if (e.data.size > 0) {
              ...
              try {
                ...
                if (json.text) {
                  pending += '\\n[AUDIO] ' + json.text;
                  allText += '\\n[AUDIO] ' + json.text;
                  addEntry('[?åÏÑ± Í∞êÏ?] ' + json.text, true);
    ...
    async function togglePip() {
      if ('documentPictureInPicture' in window) {
        ...
        try {
          ...
          // Move status panel to PiP
          pipWindow.document.body.appendChild($('statusPanel'));
          ...
          pipWindow.addEventListener('pagehide', () => {
            ...
            document.body.insertBefore($('statusPanel'), document.body.firstChild);
            pipWindow = null;
          });
        } catch (e) {
    ...
    function addEntry(text, isNew) {
      ...
      d.className = `entry ${isNew ? 'new' : ''}`;
      const time = $('clock') ? $('clock').textContent : '00:00';
      d.innerHTML = `<span class="ts">${time}</span><span class="badge ${isNew ? 'new' : 'old'}">${isNew ? '[NEW]' : '[OLD]'}</span> <span class="txt">${text.replace(/\n/g, '<br>')}</span>`;
      log.prepend(d);
      
      // keep only 100 lines to prevent UI lag
    ...
    async function runOCR() {
      ...
      try {
        ...
        for (const line of rawLines) {
          ...
          if (isStatic) {
            processedLines.push(`[Í≥†Ï†ï] ${line.text}`);
          } else {
            processedLines.push(`[?êÎßâ] ${line.text}`);
          }
        }
        ...
        for (const mem of newSpatial) {
          // Ignore very short texts like '2' or 'Í≥µÏú†'
        ...
        function updateTitleUI() {
          ...
          titleEl.textContent = `?ÑÏû¨ ?ÅÏÉÅ: ${currentVideoTitle}`;
        }
        ...
        for (const mem of newSpatial) {
           if (mem.text.length > 2) {
              ...
              if (!allText.includes(mem.text)) {
                 const tag = mem.count > 15 ? '[Í≥†Ï†ï]' : '[?êÎßâ]';
