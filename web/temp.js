...
let ocrBusy = false;
    ...
    // Initialize WebGPU Whisper Worker
    const whisperWorker = new Worker('whisper-worker.js', { type: 'module' });
    whisperWorker.postMessage({ type: 'load' });
    whisperWorker.onmessage = (e) => {
      const msg = e.data;
      if (msg.status === 'complete' && msg.text) {
        setStep('audio', 'done', 'Listening');
        let text = msg.text.trim();
        // ??“ëŽ…??Whisper???¨ì¢Žì­?ê³¸ì”¤ ?ëº¤ìŸ»/?¾ëŒ???ì¢Šï¼ˆ??•ê½•??ë€??ê¾ªê½£ï§?        const blacklisted = ["MBC", "KBS", "SBS", "YTN", "??–ê»Œ??ï¼œ?ë¶¿ê½Œ", "åª›ë¨¯ê¶??¸ë•²??, "?ë¨?­‘:", "??ˆë¸˜??°ë‚«", "??ë’ª", "?´Ñ‰ë£†??ï¼œ?ë¶¿ê½Œ"];
        if (blacklisted.some(word => text.includes(word))) return;
        ...
        $('audioCharCount').textContent = audioAllText.length + ' chars';
        maybeAnalyze();
      } else if (msg.status === 'loading') {
        setStep('audio', 'running', 'Downloading AI...');
      } else if (msg.status === 'ready') {
        setStep('audio', 'done', 'Ready (WebGPU: ' + (!msg.fallback) + ')');
      } else if (msg.status === 'error') {
        setStep('audio', 'fail', msg.message);
      }
    };
    ...
    // Variables to fix ReferenceError and implement loop detection
    ...
    async function startCapture() {
      try {
        ...
        // Pop out as floating window FIRST to preserve user gesture context
        ...
        if (audioTracks.length > 0) {
          ...
          mediaRecorder = new MediaRecorder(audioStream, { mimeType: 'video/webm' }); // Chrome uses video/webm for audio chunks sometimes
          mediaRecorder.ondataavailable = async (e) => {
            if (isVideoLooping) return; // Skip audio analysis if we are looping
            if (e.data.size > 0) {
              try {
                ...
                setStep('audio', 'running', 'Processing...');
                whisperWorker.postMessage({ type: 'transcribe', audio: float32Data });
              } catch (err) {
                setStep('audio', 'fail', 'Decode error');
          ...
          }, 8000); // 8?¥ëˆì¤???Žì ®???¾ëªƒ?????¸™ åª›ëº¥??(?ëº¥ì†—???ê³¸ë“…)
        } else {
          setStep('audio', 'fail', 'No tab audio shared');
        }
    ...
    async function runOCR() {
      ...
      if (lastBbox) {
        ...
        if (cropW > 150 && cropH > 150 && (cropW < cvs.width * 0.9 || cropH < cvs.height * 0.9)) {
          ...
          if (isVideoLooping) {
            setStep('ocr', 'skip', 'Loop Detected (Waiting for new video)');
            setTimeout(runOCR, 200);
            return;
          }

          let isLoop = false;
          // Only check for loops if the video has been playing for at least 20 seconds
          ...
          if (isLoop) {
            isVideoLooping = true;
            addEntry('========== ?ê³¸ê¸½ è«›ì„Ž????ê¹?åª›ë¨¯? (Loop) ==========', false);
            setStep('ocr', 'skip', 'Loop Detected');
            setTimeout(runOCR, 200);
            return;
          } else {
            // Store the very first 5 frames as the signature of the start of the video.
            // Sample every 2000ms to get a diverse set of frames over the first 10 seconds.
      ...
      try {
        ...
        for (const line of rawLines) {
          ...
          if (isStatic) {
            processedLines.push(`[?¨ì¢Ž?? ${line.text}`);
          } else {
            processedLines.push(`[?ë¨?­‘] ${line.text}`);
          }
        }
        ...
        for (const mem of newSpatial) {
          // Ignore very short texts like '2' or '?¨ë“­?€'
        ...
        if (maxStaticText) {
          if (!currentVideoTitle) {
            ...
            addEntry(`========== ï§??ê³¸ê¸½ åª›ë¨¯?: ${maxStaticText} ==========`, false);
            videoStartFrames = [];
        ...
        function updateTitleUI() {
          ...
          if (container) {
            container.innerHTML = `<div style="background: #1e293b; color: #60a5fa; padding: 10px 14px; border-radius: 8px; font-weight: bold; text-align: center; border: 1px solid #334155; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
              ?ê¾©ì˜± ?ê³¸ê¸½: ${currentVideoTitle.replace(/\n/g, ' ')}
            </div>`;
          }
        }
    ...
    async function analyze() {
      ...
      try {
        ...
        if (report.transcript) {
          $('rawDumpView').value = report.transcript;
        }
        if (report.preprocessedText) {
          $('preprocessedView').value = report.preprocessedText;
        }
