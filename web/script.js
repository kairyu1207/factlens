    ...
    async function startCapture() {
      try {
        ...
        if (audioTracks.length > 0) {
          ...
          setStep('audio', 'fail', 'Audio not shared');
          addAudioEntry('⚠️ 오디오가 감지되지 않았습니다. 캡처 시 "시스템 오디오 공유" 옵션을 켜주세요.');
        }
      ...
      try {
        ...
        for (const line of rawLines) {
          ...
          if (isStatic) {
            ...
            processedLines.push(`[자막] ${line.text}`);
          }
        }
        ...
        if (maxStaticText) {
          if (!currentVideoTitle) {
            ...
            addEntry(`========== 첫 영상 감지: ${maxStaticText} ==========`, false);
            videoStartFrames = [];
            ...
            if (jaccard(currentVideoTitle, maxStaticText) < 0.5) {
              ...
              $('audioFeedBody').innerHTML = '<div class="feed-empty">Waiting for audio...</div>';
        ...
        function updateTitleUI() {
          ...
          if (container) {
            container.innerHTML = `<div style="background: #1e293b; color: #60a5fa; padding: 10px 14px; border-radius: 8px; font-weight: bold; text-align: center; border: 1px solid #334155; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
              현재 영상: ${currentVideoTitle.replace(/\n/g, ' ')}
            </div>`;
          }
        }
